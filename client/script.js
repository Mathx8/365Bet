document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('finalize-bets').addEventListener('click', () => {
    ...
    fetch('http://localhost:3000/send-bet', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ betsText }),
    })
    .then(response => {
      if (response.ok) {
        alert('Apostas enviadas com sucesso para o Discord.');
      } else {
        alert('Erro ao enviar as apostas para o Discord.');
      }
    })
    .catch(error => {
